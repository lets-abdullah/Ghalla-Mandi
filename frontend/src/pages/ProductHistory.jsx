  ...
  return (
    <div className="space-y-6">
      ...
      <div className="no-print flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={onBack}
            className={`p-2 px-3 rounded-xl border transition shadow-xs cursor-pointer flex items-center gap-2 text-xs font-bold ${theme === 'dark'
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition shadow-2xs cursor-pointer ${theme === 'dark'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
        {/* On-Hand Stock */}
        <div className={`p-4 rounded-2xl border transition card-shadow ${theme === 'dark' ? 'bg-slate-800/80 border-slate-700/80' : 'bg-white border-slate-200'
          }`}>
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-[10px] font-black uppercase tracking-wider">On-Hand Stock</span>
            <div className={`w-7 h-7 rounded-xl flex items-center justify-center ${isLowStock ? 'bg-rose-500/10 text-rose-500' : 'bg-blue-500/10 text-blue-500'
              }`}>
              <Package className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className={`text-2xl font-black font-mono tracking-tight ${isLowStock ? 'text-rose-600 dark:text-rose-400' : 'text-blue-600 dark:text-blue-400'
              }`}>
      ...
      {/* ------------------------------------------------------------------------- */}
      <div className={`border rounded-2xl overflow-hidden card-shadow transition ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
        }`}>
        {/* Section Header */}
        ...
        <div className={`p-4 sm:px-5 border-t flex flex-wrap items-center justify-between gap-3 text-xs ${theme === 'dark'
