  ...
  return (
    <div className="space-y-6">
      ...
      {viewingSupplier && (() => {
        ...
        const totalPurchasedAmount = supplierPurchases.reduce((acc, p) => acc + (Number(p.amount || p.grandTotal || 0)), 0);
        const isBalanceDue = Number(viewingSupplier.balance || 0) > 0;
        ...
        return (
          <div
            ...
            className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto"
          >
            <div className={`rounded-3xl max-w-2xl w-full p-5 sm:p-6 space-y-4 card-shadow border my-auto max-h-[90vh] overflow-y-auto ${
              theme === 'dark' ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'
            }`}>
              {/* Modal Header */}
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-brand-500/20 to-brand-600/10 text-brand-600 dark:text-brand-400 flex items-center justify-center font-black text-lg border border-brand-500/20 shadow-xs">
                    {viewingSupplier.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base sm:text-lg font-black tracking-tight">{viewingSupplier.name}</h3>
                    ...
                    <p className="text-xs text-slate-400 font-semibold mt-0.5 flex items-center gap-1.5">
                      ...
                      <span>{viewingSupplier.businessName ? `${viewingSupplier.businessName} • ` : ''}Supplier Profile</span>
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* 3-Pillar Financial & Procurement KPI Banner */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {/* 1. Payable Due Balance */}
                <div className={`p-3.5 rounded-2xl border transition ${
                  ...
                  <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
                    ...
                    <DollarSign className={`w-3.5 h-3.5 ${isBalanceDue ? 'text-rose-500' : 'text-emerald-500'}`} />
                  </div>
                  <div className={`text-base sm:text-lg font-mono font-black mt-1 ${
                    isBalanceDue ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'
                  }`}>
                    Rs. {Number(viewingSupplier.balance || 0).toLocaleString()}
                  </div>
                  <div className="text-[10px] font-semibold text-slate-400 mt-0.5">
                    {isBalanceDue ? 'Outstanding Amount' : '✓ All Dues Cleared'}
                  </div>
                </div>

                {/* 2. Total Procured Purchases */}
                <div className={`p-3.5 rounded-2xl border ${
                  ...
                  <div className="text-base sm:text-lg font-mono font-black mt-1 text-blue-600 dark:text-blue-400">
                    Rs. {totalPurchasedAmount.toLocaleString()}
                  </div>
                  <div className="text-[10px] font-semibold text-slate-400 mt-0.5">
                    {supplierPurchases.length} Purchase Invoices
                  </div>
                </div>

                {/* 3. Supplied Commodities */}
                <div className={`p-3.5 rounded-2xl border ${
                  theme === 'dark' ? 'bg-slate-900/60 border-slate-700' : 'bg-slate-50 border-slate-200'
                }`}>
                  <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
                  ...
                  <div className="text-base sm:text-lg font-mono font-black mt-1 text-purple-600 dark:text-purple-400">
                    {(viewingSupplier.suppliedProducts || []).length} Items
                  </div>
                  <div className="text-[10px] font-semibold text-slate-400 mt-0.5">
              ...
              {/* 2-Column Info Grid: Vendor & Contact vs Bank Details */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                {/* Vendor & Contact Information Card */}
                <div className={`p-4 rounded-2xl border space-y-2.5 text-xs ${
                  theme === 'dark' ? 'bg-slate-900/40 border-slate-700' : 'bg-slate-50/80 border-slate-200'
                }`}>
                  <div className="text-[11px] font-black uppercase tracking-wider text-brand-600 dark:text-brand-400 flex items-center gap-1.5 border-b border-slate-200 dark:border-slate-700/80 pb-1.5">
                  ...
                  <div className="flex justify-between items-center text-slate-500">
                    <span className="flex items-center gap-1.5">
                      <MapPin className="w-3 h-3 text-slate-400" /> City / Mandi:
                    </span>
                  ...
                  {viewingSupplier.address && (
                    <div className="flex justify-between items-start text-slate-500 pt-1 border-t border-slate-200 dark:border-slate-700/60">
                      <span className="shrink-0">Address:</span>
                      <span className="font-medium text-right text-slate-800 dark:text-slate-200 pl-2">
                        {viewingSupplier.address}
                      </span>
                    </div>
                  )}
                ...
                <div className={`p-4 rounded-2xl border space-y-2.5 text-xs ${
                  ...
                  <div className="text-[11px] font-black uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5 border-b border-slate-200 dark:border-slate-700/80 pb-1.5">
                    <Landmark className="w-3.5 h-3.5" />
                  ...
                  <div className="flex justify-between items-center text-slate-500">
                    <span>Bank Name:</span>
                    <span className="font-bold text-slate-900 dark:text-white">
                      {viewingSupplier.bankName || 'Not Provided'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center text-slate-500">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">
                      {viewingSupplier.accountTitle || viewingSupplier.name}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center text-slate-500">
                    ...
                    <span className="font-mono font-bold text-slate-900 dark:text-white">
                      {viewingSupplier.accountNumber || viewingSupplier.iban || 'N/A'}
                    </span>
                  </div>

                  <div className="flex justify-between items-center text-slate-500 pt-1 border-t border-slate-200 dark:border-slate-700/60">
                    <span>Payment Terms:</span>
                    <span className="font-bold text-brand-600 dark:text-brand-400">
                      Standard Mandi Ledger
                    </span>
                  </div>
                </div>
              </div>

              {/* Supplied Products Chips Card */}
              <div className={`p-3.5 rounded-2xl border space-y-2 ${
                ...
                <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>Supplied Commodities & Products</span>
                  </span>
                  <span className="text-[10px] text-slate-400">
                    {(viewingSupplier.suppliedProducts || []).length} registered
                  </span>
                </div>

                <div className="flex flex-wrap gap-1.5">
                  {(viewingSupplier.suppliedProducts || []).length === 0 ? (
                    <span className="text-xs text-slate-400 italic">All general mandi commodities</span>
                  ) : (
                ...
                {viewingSupplier.notes && (
                  <div className="text-xs text-slate-500 pt-2 border-t border-slate-200 dark:border-slate-700/60 flex items-center gap-1.5">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-3 py-2 rounded-xl border text-xs font-bold transition cursor-pointer flex items-center gap-1.5 ${
                      theme === 'dark' ? 'border-slate-700 hover:bg-slate-700 text-slate-300' : 'border-slate-200 hover:bg-slate-100 text-slate-700'
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl transition shadow-md shadow-emerald-600/20 cursor-pointer flex items-center gap-1.5"
                  >
